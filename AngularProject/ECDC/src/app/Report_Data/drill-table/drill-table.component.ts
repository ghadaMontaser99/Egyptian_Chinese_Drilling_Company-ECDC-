...
@Component({
  selector: 'app-drill-table',
  templateUrl: './drill-table.component.html',
  styleUrls: ['./drill-table.component.scss']
})
export class DrillTableComponent {

  private ListOfDrill = new BehaviorSubject<any[]>([]);
  readonly DrillList = this.ListOfDrill.asObservable();
  page:number=1;
  count:number=0;
  ...
  constructor(private AddnewDrill: AddNewDrillServiceService,private getDataByPage:GetdataByPageService,private dataService: DataService, private deleteService: DeleteDataService,private loginService:LoginService) { }
  ...
  DeleteDrill(Drill: IDrill) {
    if (confirm("Are you sure you want to delete this Record?")) {
      // user clicked "Yes"
      // do something, such as call an API to delete the item
      this.deleteService.DeleteDrill(Drill).subscribe({
        next: data => {
          console.log(data)
  ...
  getpages(num:number)
  {
    this.getDataByPage.GetDrillByPage(num,this.User.ID,this.User.Role).subscribe({
      next: data => {
        console.log(' this.ListOfDrill', this.ListOfDrill)
        this.ListOfDrill.next(data.items)
        this.countOfPage=data.count;
        this.TempArray= new Array(this.countOfPage);
      },
      error: err => {
        console.log(' this.ListOfDrill', this.ListOfDrill.value)
