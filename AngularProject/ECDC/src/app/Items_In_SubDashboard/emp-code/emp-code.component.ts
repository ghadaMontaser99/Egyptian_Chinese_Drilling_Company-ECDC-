...
@Component({
  selector: 'app-emp-code',
  templateUrl: './emp-code.component.html',
  styleUrls: ['./emp-code.component.scss']
})
export class EmpCodeComponent {
  private ListOfCodes = new BehaviorSubject<IEmpCode[]>([]);
  readonly EmpCodeList = this.ListOfCodes.asObservable();
  page: number = 1;
  count: number = 0;
  ...
  DeleteEmpCode(EmpCode: IEmpCode) {
    if (confirm("Are you sure you want to delete this Record?")) {
      // user clicked "Yes"
      // do something, such as call an API to delete the item
      this.deleteService.DeleteEmpCode(EmpCode).subscribe({
        next: data => {
          console.log(data)
  ...
  getpages(num: number) {
    this.getDataByPage.GetEmpCodeByPage(num).subscribe({
      next: data => {
        this.ListOfCodes.next(data.items)
  ...
  Download() {
    ...
    let worksheet = workbook.addWorksheet("Emp.NO Data");
